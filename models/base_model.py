...
from models.layers import ColumnMaxPooling, RegularBlock
    ...
    def __init__(
        ...
        for i in range(self.num_blocks - 1):
            mlp_block = RegularBlock(
                last_layer_features, in_features, self.depth_of_mlp, self.freeze_mlp[i]
            )
            self.reg_blocks.append(mlp_block)
            last_layer_features = in_features
        mlp_block = RegularBlock(
            last_layer_features, out_features, depth_of_mlp, self.freeze_mlp[self.num_blocks - 1]
        )
        self.reg_blocks.append(mlp_block)
...
class Simple_Edge_Embedding(nn.Module):
    def __init__(
        ...
        self.depth_of_mlp = depth_of_mlp
        self.base_model = BaseModel(
            original_features_num, num_blocks, in_features, in_features, depth_of_mlp, freeze_mlp
        )
